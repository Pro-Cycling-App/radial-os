...
// --- Extra Gatekeepers -------------------------------------------------------------------------
// This wrapper's one addition to upstream's deployment shape. See EXTRA_GATEKEEPERS in deploy.ts.

/** {@link baseConfigs} plus the upstream base configs of the Gatekeepers these tests configure. */
async function basesWithGatekeepers(): Promise<BaseConfigs> {
  return {
    ...await baseConfigs(),
    gatekeepers: {
      mcp: await baseConfig("../cloudflare-os/packages/gatekeeper-mcp/wrangler.jsonc"),
      mcp_portal:
        await baseConfig("../cloudflare-os/packages/gatekeeper-mcp-portal/wrangler.jsonc"),
    },
  };
}

// mcp_portal earns its place in every one of these: it is the only id whose binding name and router
// path differ, and the only one carrying vars of its own.
function withGatekeepers(): DeploymentConfig {
...
test("binds each extra Gatekeeper to both the Workshop and the router", async () => {
  const generated = generateConfigs(withGatekeepers(), await basesWithGatekeepers());

  assert.deepEqual(
    generated.workshop.services?.find((service) => service.binding === "GATEKEEPER_MCP_PORTAL"),
    {
      binding: "GATEKEEPER_MCP_PORTAL",
      service: "acme-cloudflare-os-mcp-portal",
      entrypoint: "GatekeeperVendor",
    });
  // No entrypoint on the router: it forwards whole HTTP requests, and the binding name is what
  // picks the path.
  assert.deepEqual(
    generated.router.services?.find((service) => service.binding === "GATEKEEPER_MCP_PORTAL"),
    { binding: "GATEKEEPER_MCP_PORTAL", service: "acme-cloudflare-os-mcp-portal" });
});

test("derives each extra Gatekeeper's BASE_URL from the router's path for it", async () => {
  const generated = generateConfigs(withGatekeepers(), await basesWithGatekeepers());

  // The router lowercases the binding suffix and hyphenates it, so GATEKEEPER_MCP_PORTAL is served
  // at /gatekeeper/mcp-portal. A BASE_URL of /gatekeeper/mcp_portal would deploy happily and then
  // send OAuth callbacks to a path nothing serves.
...
test("merges configured vars over an extra Gatekeeper's base config", async () => {
  const generated = generateConfigs(withGatekeepers(), await basesWithGatekeepers());
  ...
  // A base-config var nobody overrode survives the merge.
  assert.equal(generated["gatekeeper-mcp"]?.vars?.MCP_ALLOW_INSECURE, "false");
});

test("leaves the Workers alone when no extra Gatekeeper is configured", async () => {
  const generated = generateConfigs(validConfig, await baseConfigs());

  assert.equal(generated["gatekeeper-mcp"], undefined);
  assert.ok(!generated.router.services?.some((service) =>
    service.binding.startsWith("GATEKEEPER_MCP")));
});

test("rejects an unusable extra Gatekeeper block", () => {
  assert.throws(() => validateConfig(variant((config) => {
    config.gatekeepers = { slack: { name: "acme-cloudflare-os-slack" } };
  })), /Unknown gatekeeper "slack"/);

  assert.throws(() => validateConfig(variant((config) => {
    config.gatekeepers = { mcp: { name: "" } };
  })), /gatekeepers\.mcp\.name/);

  assert.throws(() => validateConfig(variant((config) => {
    config.gatekeepers = { mcp: { name: "acme-cloudflare-os-mcp", vars: { PORT: 42 } } };
  })), /gatekeepers\.mcp\.vars/);

  // Derived, never configured: the router's path for the Gatekeeper and the URLs the Gatekeeper
  // builds have to agree.
  assert.throws(() => validateConfig(variant((config) => {
    config.gatekeepers = {
      mcp: { name: "acme-cloudflare-os-mcp", vars: { BASE_URL: "https://os.example.com/mcp" } },
    };
  })), /BASE_URL/);

  // One namespace with workers.*: a shared name overwrites that Worker rather than adding one.
  assert.throws(() => validateConfig(variant((config) => {
    config.gatekeepers = { mcp: { name: "acme-cloudflare-os-backend" } };
  })), /unique/);
});

test("rebuilds each extra Gatekeeper's configurator from source", () => {
  const commands = buildCommands(withGatekeepers()).map(({ args }) => args.join(" "));
  const portal = commands.filter((command) =>
    command.includes("@gadgets/mcp-portal-gatekeeper"));

  // build:configurator is named explicitly rather than reached through `build`'s dependency: it is
  // the step that reads VITE_FRONTEND_ERROR_REPORTING into the generated UI, so a replay from a
  // fingerprint built under the other value is a wrong artifact, not a stale one.
  assert.ok(portal.some((command) => command.endsWith("--no-cache build:configurator")));
  assert.ok(portal.some((command) => command.endsWith("--no-cache build")));
});
