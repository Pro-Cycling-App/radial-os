...
/**
 * An upstream Gatekeeper this wrapper deploys beyond the set upstream configures itself.
 *
 * Keyed by vendor id rather than by Worker name, because the id is load-bearing twice: it selects
 * the submodule package to build and deploy, and -- uppercased -- it becomes the `GATEKEEPER_<ID>`
 * service binding whose name the router turns back into a `/gatekeeper/<id>` path.
 *
 * These are upstream's Gatekeepers, not custom ones. `packages/custom-gatekeeper` is this
 * repository's own; these live in the submodule and are simply outside the *ambient* set (Context
 * and Scheduler) that upstream marks `PREINSTALL` and this starter therefore deploys
 * unconditionally. See `EXTRA_GATEKEEPERS` in `deploy.ts` for the packages behind the ids.
 */
export type ExtraGatekeeperId = "github" | "linear" | "mcp" | "mcp_portal" | "email";

/** One entry in the `gatekeepers` block. */
export interface ExtraGatekeeperConfig {
  /** Permanent Worker service identity, unique across every Worker in the deployment. */
  name: string;
  /**
   * Plain, non-secret vars for this Gatekeeper, merged over its base config's own.
   *
   * `BASE_URL` is rejected here: it is derived from the deployment's public origin and the
   * router's path for this Gatekeeper, and the two must agree. OAuth client secrets are not vars
   * at all -- install them on the Worker with `wrangler secret put`.
   */
...
export interface DeploymentConfig {
  ...
  /**
   * Additional upstream Gatekeepers to deploy. Absent or `{}` deploys only the Workers above.
   * This block is this wrapper's one addition to upstream's `deployment.jsonc` shape.
   */
  gatekeepers?: Partial<Record<ExtraGatekeeperId, ExtraGatekeeperConfig>>;
  access: AccessConfig;
...
export interface GeneratedConfigs {
  ...
  /**
   * One entry per configured extra Gatekeeper, keyed `gatekeeper-<id>` so it matches the key
   * `deploy.ts` files its package directory and generated config path under. None of the named
   * keys above match this pattern, so they keep their own exact types.
   */
  [extra: `gatekeeper-${string}`]: ProdWranglerConfig | undefined;
}
...
export interface BaseConfigs {
  ...
  customGatekeeper: ProdWranglerConfig;
  errorReporter: ProdWranglerConfig;
  /** Read from the submodule for each configured extra Gatekeeper only. */
  gatekeepers?: Partial<Record<ExtraGatekeeperId, ProdWranglerConfig>>;
}
