...
/**
 * The upstream Gatekeepers this wrapper can deploy, beyond the Workers upstream configures itself.
 *
 * Each lives in the submodule, so this table is the whole of the wrapper's knowledge of them: the
 * directory to deploy from, and the pnpm package to build. Adding one to `deployment.jsonc`
 * deploys its Worker, binds it to the Workshop for vendor RPC, and binds it to the router -- which
 * is what publishes it at `/gatekeeper/<id>`.
 *
 * Upstream owns the code; this repository owns only the decision to run it and under which Worker
 * name. Anything genuinely ours belongs in `packages/`, per docs/customization.md#code-extensions.
 */
const EXTRA_GATEKEEPERS: Record<ExtraGatekeeperId, { dir: string; pkg: string }> = {
  ...
  mcp_portal: {
    ...
    pkg: "@gadgets/mcp-portal-gatekeeper",
  },
  email: { dir: "cloudflare-os/packages/gatekeeper-email", pkg: "@gadgets/email-gatekeeper" },
};

// Every directory a generated `wrangler.prod.jsonc` can land in. The extra Gatekeepers are all
// listed, not just the configured ones, so the cleanup in `main` removes a file left behind by a
// previous run whose `deployment.jsonc` configured more of them than this one does.
const generatedPaths: Record<string, string> = Object.fromEntries(
  Object.entries({
    ...packageDirs,
    ...Object.fromEntries(
      Object.entries(EXTRA_GATEKEEPERS).map(([id, { dir }]) => [`gatekeeper-${id}`, dir])),
  }).map(([name, dir]) => [name, join(root, dir, generatedName)]));

/**
 * The configured extra Gatekeepers, as `[id, config]` pairs.
 *
 * The predicate is what keeps the id typed without a cast. It can never actually drop an entry on
 * the deploy path: `validateExtraGatekeepers` rejects an unknown id outright, and it runs before
 * anything here is read.
 */
function extraGatekeepers(
  config: DeploymentConfig,
): [ExtraGatekeeperId, ExtraGatekeeperConfig][] {
  return Object.entries(config.gatekeepers ?? {}).filter(
    (entry): entry is [ExtraGatekeeperId, ExtraGatekeeperConfig] =>
      entry[0] in EXTRA_GATEKEEPERS && entry[1] !== undefined);
}

/**
 * The service-binding name for a Gatekeeper, and the router path derived back from it.
 *
 * The router discovers Gatekeepers by scanning `GATEKEEPER_*` bindings, lowercasing the suffix and
 * turning underscores into hyphens (`GATEKEEPER_MCP_PORTAL` serves `/gatekeeper/mcp-portal`).
 * `BASE_URL` has to survive the same round trip: a Gatekeeper builds its OAuth redirect URIs and
 * resource URLs from it, so a value that disagrees with the router points them at a path nothing
 * serves -- and OAuth fails at the callback, after the user has already approved. Both are derived
 * here from the one id so they cannot drift apart.
 */
function gatekeeperBinding(id: ExtraGatekeeperId): string {
  return `GATEKEEPER_${id.toUpperCase()}`;
}

function gatekeeperPath(id: ExtraGatekeeperId): string {
  return `/gatekeeper/${id.replaceAll("_", "-")}`;
}
...
/**
 * The `gatekeepers` block: this wrapper's only addition to `deployment.jsonc`'s shape.
 *
 * Names are checked by the caller against the same rules and in the same namespace as `workers.*`.
 * That shared namespace is the point: a Gatekeeper sharing a name with the Workshop would not be a
 * second Worker, it would overwrite the first.
 */
function validateExtraGatekeepers(config: DeploymentConfig): void {
  const gatekeepers = config.gatekeepers;
  if (gatekeepers === undefined) return;
  if (gatekeepers === null || typeof gatekeepers !== "object" || Array.isArray(gatekeepers)) {
    throw new Error("gatekeepers must be an object keyed by vendor id.");
  }
  for (const [id, gatekeeper] of Object.entries(gatekeepers)) {
    if (!(id in EXTRA_GATEKEEPERS)) {
      throw new Error(
        `Unknown gatekeeper "${id}". Known: ${Object.keys(EXTRA_GATEKEEPERS).join(", ")}.`);
    }
    if (!gatekeeper || typeof gatekeeper.name !== "string" || !gatekeeper.name) {
      throw new Error(`gatekeepers.${id}.name must be a non-empty string.`);
    }
    const vars = gatekeeper.vars;
    ...
    if (vars && "BASE_URL" in vars) {
      throw new Error(
        `gatekeepers.${id}.vars.BASE_URL is derived from the deployment's public origin and the ` +
        "router's path for this Gatekeeper, which have to agree. Remove it.");
    }
  }
}
...
export function validateConfig(config: DeploymentConfig): DeploymentConfig {
  ...
  // Before the name checks below, which fold the Gatekeeper names into the same namespace and so
  // need them already known to be strings.
  ...
  const workerNames = [
    ...
    ...extraGatekeepers(config).map(([, gatekeeper]) => gatekeeper.name),
  ];
  if (new Set(workerNames).size !== workerNames.length) {
    throw new Error(
      "Router, Workshop, Context, Scheduler, custom Gatekeeper and extra Gatekeeper names must " +
      "be unique.");
  }
...
export function generateConfigs(config: DeploymentConfig, bases: BaseConfigs): GeneratedConfigs {
  ...
  const extras = extraGatekeepers(config);
  ...
  router.services = [
    ...
    ...extras.map(([id, { name }]) => ({ binding: gatekeeperBinding(id), service: name })),
  ];
  ...
  workshop.services = [
    ...
    // Twice each, like Context and Custom above: the vendor entrypoint here for RPC, and the
    // plain binding on the router for the HTTP half (OAuth callbacks, resource URLs).
    ...extras.map(([id, { name }]) => ({
      binding: gatekeeperBinding(id),
      service: name,
      entrypoint: "GatekeeperVendor",
    })),
  ];
  ...
  // The extra Gatekeepers. Nothing but the common block, the deployment's own vars, and the
  // derived BASE_URL: everything else about them is upstream's base config.
  const extraConfigs: Record<string, ProdWranglerConfig> = {};
  for (const [id, { name, vars }] of extras) {
    const base = bases.gatekeepers?.[id];
    if (!base) {
      throw new Error(`No base wrangler config was read for gatekeeper "${id}".`);
    }
    const gatekeeper = structuredClone(base);
    setCommon(gatekeeper, config, name);
    gatekeeper.vars = { ...gatekeeper.vars, ...vars, BASE_URL: `${origin}${gatekeeperPath(id)}` };
    extraConfigs[`gatekeeper-${id}`] = gatekeeper;
  }
...
export function buildCommands(config: DeploymentConfig): BuildCommand[] {
  return [
    ...
    // The extra Gatekeepers, in the same two-step shape as Context and the Scheduler above but for
    // a different reason. Their `build` is a Vite+ *task* that only type-checks, declaring
    // `build:configurator` as a dependency; `build:configurator` is what generates the configurator
    // UI, and it is the step that reads VITE_FRONTEND_ERROR_REPORTING and so must not replay from a
    // fingerprint built under another value. Naming it explicitly is what each package's own
    // `deploy` script does (`vp run --no-cache build:configurator && wrangler deploy`), rather than
    // trusting `--no-cache` on `build` to reach through the dependency. `build` then runs `tsc`
    // over what this step just wrote.
    ...extraGatekeepers(config).flatMap(([id]) => [
      { args: submoduleBuild(EXTRA_GATEKEEPERS[id].pkg, "build:configurator") },
      { args: submoduleBuild(EXTRA_GATEKEEPERS[id].pkg) },
    ]),
...
async function main(): Promise<void> {
  ...
  const generated = generateConfigs(config, {
    ...
    gatekeepers: Object.fromEntries(await Promise.all(
      extraGatekeepers(config).map(async ([id]) =>
        [id, await readJsonc<ProdWranglerConfig>(
          join(root, EXTRA_GATEKEEPERS[id].dir, "wrangler.jsonc"))] as const))),
  });
  ...
  try {
    for (const [name, generatedConfig] of Object.entries(generated)) {
      ...
      const path = generatedPaths[name];
      if (!path) throw new Error(`No package directory is known for generated config "${name}".`);
    ...
    for (const [id] of extraGatekeepers(config)) {
      deployWorker(EXTRA_GATEKEEPERS[id].dir, deployArgs);
    }
    // After every Gatekeeper: it binds them all.
